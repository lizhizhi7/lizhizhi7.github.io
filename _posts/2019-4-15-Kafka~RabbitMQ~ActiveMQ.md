...
![RabbitMQ](https://pic.intellizhi.cn/blogimg/1.jpg)

此时，其实等同于ActiveMQ的queue模式。

在RabbitMQ里，一个queue可以有多个消费者

![RabbitMQ](https://pic.intellizhi.cn/blogimg/2.jpg)
...
![RabbitMQ](https://pic.intellizhi.cn/blogimg/rabbitmq3.jpg)

总体说来fanout其实就是direct交换器实现的。把两个队列都绑定到direct，绑定的时候指定同一个key，就变成fanout交换器了，如下图

![RabbitMQ](https://pic.intellizhi.cn/blogimg/rabbitmq4.jpg)

queue和exchange绑定的时候，也可以指定多个绑定key，这时候就实现了简单版的订阅。如下图

![RabbitMQ](https://pic.intellizhi.cn/blogimg/raabitmq5.jpg)

当然这样不够灵活，我想要靠通配符绑定如何呢，这时候就不用direct交换器了，用topic交换器

![RabbitMQ](https://pic.intellizhi.cn/blogimg/rabbitmq6.jpg)
...
![Kafka](https://pic.intellizhi.cn/blogimg/kafka1.jpg)
...
![Kafka](https://pic.intellizhi.cn/blogimg/kafka2.jpg)
